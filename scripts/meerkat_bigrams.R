...
setwd("C:/Users/vdemartsev/My Cloud/HyraxPostDoc/Call_sequences/labels")
...
call_data <- read.csv(paste(getwd(), "/", specie, "/foc_calls_resolved.csv", sep = ""))
...
self <- ggplot(longData, aes(x = variable, y = COLLOCATE)) + 
  ...
  labs(x="Responce", y="Initiation", title="Self replies") +
...
non_self <- ggplot(longData, aes(x = variable, y = COLLOCATE)) + 
       geom_raster(aes(fill= value)) + 
       scale_fill_gradient(low="grey90", high="red") +
       labs(x="Responce", y="Initiation", title="Non self reply") +
       geom_text(  aes(x=variable, y=COLLOCATE, label = r_val), color="black", size=5) + 
       theme_bw() + theme(axis.text.x=element_text(size=12, angle=0, vjust=0.3),
                          axis.text.y=element_text(size=12),
                          plot.title=element_text(size=14))
...
sn_comb <- c("sn_mo", "mo_sn", "sn_ld", "ld_sn", "sn_al", "al_sn")

(sum(frequent_call_pairs[which(frequent_call_pairs$Var1 %in% sn_comb), "Freq"]) / sum(frequent_call_pairs$Freq))*100
