...
for (date in 1:length (dates)) {
  ...
  for (individual in individuals) {
    ...
    #start row counter
      #setting it at one to start from the first row of data 
      #it will dynamically change dependent of the conditions in the while loop
    i <- 1
    
    while (i < nrow(individual_select)) { #make sure that i is not going beyond the number of rows in data 
      ...
      #set t0 as the time the meerkat arrived to a new patch
       ...
       if (t0 > individual_select$t[nrow(individual_select)]) {break} #if t0 goes beyond the relevant data window break the loop
      ...
      if (length(arrival_row) == 0) {
        ...
        #avoid quick passes through the patch by setting min time in patch to 30 sec
        if (fut_step_dur < 30) {
          i <- i + 1
       ...
       #####combine all call rate data together ####
          ...
          #summarize  data for general call rates  
     all_arrival_points <- bind_rows(all_arrival_points,               
          bind_cols(
            individual_select[arrival_row, ],
            (nrow(calls_in_window)/tw)-base_call_rate,  #subtract base call rate to control for individual variation (maybe using Z score will be better)
            "call_rate",
            tw*win
          ))
     
            #summarize data for Close call rates
     all_arrival_points <- bind_rows(all_arrival_points,               
                                     bind_cols(
                                       individual_select[arrival_row, ],
                                       (nrow(cc_calls_in_window)/tw)-base_cc_rate, #subtract base cc rate to control for individual variation 
                                       "cc_call_rate",
                                       tw*win
                                     ))
     
             #Summarize data for Short note call rates   
     all_arrival_points <- bind_rows(all_arrival_points,               
                                     bind_cols(
                                       individual_select[arrival_row, ],
                                       (nrow(sn_calls_in_window)/tw)-base_sn_rate, #subtract base sn rate to control for individual variation
                                       "sn_call_rate",
                                       tw*win
                                     ))
        ...
        #set the i row counter the current arrival row to keep the while condition going
        i <-  arrival_row 
      }
...
#calculate speed of arrival to the currents patch. How fast the individual aproached the curretnt patch?
...
med_high <- as.numeric(quantile (all_arrival_points$indSpeedPast, probs = seq(0, 1, 0.1), na.rm= T)[7])
 ...
 #this is the main plot showing diferences in call rate (y) as a function of time spent in patch (x) 
 #for the three categories of speed of arrival (slow = green, medium = orange, fast = blue)
ggplot(data = all_arrival_points[which(all_arrival_points$indSpeedPas < slow),], aes(x = time_window, y = rate)) + geom_hline(yintercept=0, linetype="dashed", color = "black", size= 1) + 
        ...
        facet_wrap(~ type) + scale_x_continuous(sec.axis = sec_axis(~ . , name = "Call_rate vs time spent in the patch", breaks = NULL, labels = NULL)) +
        theme_minimal() 

### correcting the plot for neater plotting ###
### and also easyer stats ####
all_arrival_points[which(all_arrival_points$indSpeedPas < slow),"SOA"] <- "slow"
all_arrival_points[which(all_arrival_points$indSpeedPas > fast), "SOA"] <- "fast"
all_arrival_points[which(all_arrival_points$indSpeedPas > med_low & all_arrival_points$indSpeedPas < med_high), "SOA"] <- "medium"
...
ggplot(data = all_arrival_points, aes(x = time_window, y = rate, linetype = SOA)) + #geom_hline(yintercept=0, linetype="dashed", color = "black", size= 1) + 
  geom_smooth(color = "black", method = "gam", alpha=0.2, level=0.95)+ xlab("sec*10") + ylab("call rate (calls/sec)")  + facet_wrap(~ type, labeller=facet_labeller) + 
...
model_CC <- glmmTMB(data = all_arrival_points[which(all_arrival_points$type == "cc_call_rate"),] , rate ~ time_window*SOA + (1|indUniqID))
summary(model_CC)

model_sn <- glmmTMB(data = all_arrival_points[which(all_arrival_points$type == "sn_call_rate"),] , rate ~ time_window*SOA + (1|indUniqID))
summary(model_sn)
